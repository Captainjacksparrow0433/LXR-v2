...
const { getBuffer, igStory, downVideo } = require("../download");
...
Asena.addCommand({pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram" , dontAddCommandList: true }, async (message, match) => {
   ...
   const { status, type, data } = await instagram(match[1], 'julie')
    if (!status) return await message.sendMessage('✅️Example : https://www.instagram.com/reel/CSvyQoshQph/?utm_medium=copy_link\n\n*Change /reel/ to /p/ Then give Command .insta*\n\n*Any doubt ask to my owner*\n\n*contact*\n\n http://wa.me/919061103727')

    reply = await message.client.sendMessage(message.jid,UPLOAD_ING , MessageType.text, { quoted: message.data });
    
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "*Made by captain*", quoted: message.data })
    
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "*Made by captain*", quoted: message.data })
    
});
    
// thanks to :- farhan-dqz

Asena.addCommand({ pattern: 'story ?(.*)', fromMe: true, desc: "Download stories of given username\n.story username or reply to a username or story url"},
  async (message, match) => {
    ...
    if (
      ...
      return await message.sendMessage(message.jid, LOAD_ING , MessageType.text, { quoted: message.data });
    ...
    if (json.medias.length > 0) {
      await message.sendMessage(message.jid,UPLOAD_ING , MessageType.text, { quoted: message.data });
      );
