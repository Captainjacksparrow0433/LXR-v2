...
if (Config.ALIVETYPE == 'two') {

Jacki.addCommand({pattern: 'alive', fromMe: false, desc: Lang.ALIVE_DESC}, (async (message, match) => {
...
var i = Math.floor(31*Math.random())
    ...
    if (Config.ALIVEMSG.includes('{pp}')) {
        ...
        await axios.get(pp, {responseType: 'arraybuffer'}).then(async (res) => { await message.client.sendMessage(message.jid, res.data, MessageType.image, { caption: Config.ALIVEMSG.replace('{pp}', '').replace('•', r_text[i])}); });
	}    
	   else {
		   
		   var a_plk = new Array ();

        a_plk[0] = "https://telegra.ph/file/ce038342b49656b7cef44.jpg";
        ...
        a_plk[5] = "https://telegra.ph/file/ce038342b49656b7cef44.jpg";
        ...
        a_plk[10] = "https://i.ibb.co/qWnhqVf/images-14.jpg";
	 var p = Math.floor(11*Math.random())

        var plk_alive = await axios.get(`${a_plk[p]}`, { responseType: 'arraybuffer' })

        await message.client.sendMessage(message.jid, Buffer(plk_alive.data), MessageType.image, {mimetype: Mimetype.png, caption: Config.ALIVEMSG.replace('•', r_text[i])})
	   }
    }));
    }
